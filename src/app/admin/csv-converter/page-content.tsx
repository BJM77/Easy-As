...
import { Loader2, UploadCloud, FileJson, Download, Eye, Search, MapPin, ArrowRight, PlusCircle, Trash2, Save, FileType, Info } from 'lucide-react';
...
const initialConverterStates: Record<string, ConverterState> = {
  ...
  ], true, "Expected Format: Upload the standard Toll B2B IPEC Excel file. The system translates numeric zones (e.g., 200) to text (SYD) if the translation toggle is on. It combines 'Parcel' with Origin (Col F) and Destination (Col G) to create the unique Logic key."),
  ...
  ], true, "Expected Format: Upload the LCP IPEC spreadsheet. Ensure that the Origin zone is in Column B and Destination is in Column C. The Basic Charge is expected in Column E and Kilo Rate in Column J."),
  priority: createInitialConverterState('B2B Priority', 'b2b_priority', '2', [
    ...
    { id: generateMappingId(), jsonKey: 'B1', excelMapping: '{L}', placeholder: 'Basic Charge' },
    { id: generateMappingId(), jsonKey: 'K1', excelMapping: '{N}', placeholder: 'Kilo Rate' }
  ], true, "Expected Format: Upload the standard Toll Priority Excel file. The system combines the prefix '02 02' with Origin (Col F) and Destination (Col G) to construct the Priority Logic key."),
  ...
  ], false, "Expected Format: Upload the B2C rates spreadsheet. The system expects tiered pricing at specific columns (e.g., 1kg in R, 3kg in T). Turn off 'Translate Numeric Zones' if your zones are already in text format."),
  ...
  ], false, "Expected Format: Upload standard Pallet rates. Ensure the Origin PE Zone is mapped to Column F and Destination PE Zone to Column G."),
  ...
  ], false, "Expected Format: Upload the West-East special rates. This relies on Destination City Names in Column G rather than typical zone identifiers."),
};
      ...
      const transformedData = dataRows.map(row => {
        ...
        columnMapping.forEach(mapItem => {
          if (mapItem.jsonKey.trim()) {
            ...
            const columnPlaceholders = (mapItem.excelMapping.match(/{([A-Z]+)}/g) || []) as string[];
          ...
          <AccordionItem value={key} key={key} className="border rounded-lg bg-card">
            ...
            <AccordionContent className="pt-0 px-4 pb-4">
              <div className="space-y-4">
                {converter.guideText && (
                  <div className="bg-muted/50 p-4 rounded-md border text-sm text-muted-foreground flex gap-3">
                    <Info className="h-5 w-5 text-primary shrink-0" />
                    <div>
                      <strong className="text-foreground">How to use &amp; Expected Format:</strong> <br />
                      {converter.guideText}
                    </div>
                  </div>
                )}
