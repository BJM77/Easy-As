...
const calculateBulkFreightPrice = async (
    ...
    // Custom chargeable weight calculation for the Bulk page
    const calculateBulkChargeableWeight = (items: FreightItem[]): { chargeableWeight: number, rawCubicWeight: number, deadWeight: number, reductionApplied: boolean, reductionPercent: number } => {
    ...
    const { chargeableWeight, rawCubicWeight, deadWeight, reductionApplied, reductionPercent } = calculateBulkChargeableWeight(formData.items);
...
export default function BulkPageContent() {
  ...
  const onSubmit = async (data: FreightFormValues) => {
    ...
    const result = await calculateBulkFreightPrice(
        { ...data, spendBand: '4' },
        serviceSettings,
  ...
  const handleEmailSubmission = () => {
    ...
    const subject = `New Bulk Freight Booking: ${formData.originQuery} to ${formData.destinationQuery}`;
    let body = "A new bulk freight movement has been submitted for actioning.\n\n";
    body += "---CONSIGNMENT DETAILS---\n";
    body += `Origin: ${formData.originQuery}\n`;
    body += `Destination: ${formData.destinationQuery}\n`;
    body += `Service: B2B Priority (Bulk Rate)\n\n`;
    ...
    body += "---PRICING---\n";
    if (calculatedResult) {
        body += `Bulk Rate Price (ex GST): ${new Intl.NumberFormat('en-AU', { style: 'currency', currency: 'AUD' }).format(calculatedResult.finalPrice ?? 0)}\n`;
        body += `Chargeable Weight: ${calculatedResult.chargeableWeight.toFixed(2)} kg\n`;
    }
    if (calculatedFridayResult) {
        body += `FRIDAY Bulk Rate Price (ex GST): ${new Intl.NumberFormat('en-AU', { style: 'currency', currency: 'AUD' }).format(calculatedFridayResult.finalPrice ?? 0)}\n`;
    }
    body += "\n---END---\n";
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="flex flex-wrap gap-2">
                <Button type="submit" className="text-lg py-3 px-6 bg-accent hover:bg-accent/90 text-accent-foreground flex-grow md:flex-grow-0" disabled={overallLoading}>
                  {overallLoading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Calculating...</>) : 'Calculate Bulk Rate'}
                </Button>
        ...
        <div ref={resultsRef} className="mt-8 space-y-6">
            {showResults && calculatedResult && (
                 <Card className="card-print">
                    <CardHeader>
                        <CardTitle className="flex items-center text-xl font-semibold"><Package className="mr-2"/>B2B Priority Bulk Result</CardTitle>
