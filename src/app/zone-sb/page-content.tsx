...
import { Loader2, Scale, PlusCircle, Trash2, ArrowRight, CheckCircle2, AlertCircle, TrendingDown, Settings, User, Briefcase, Printer, BarChart, Eraser, Download, UploadCloud, MapIcon, Calculator, Sparkles, Mail, Clipboard, Check, Lock } from 'lucide-react';
...
type ZoneSBLeg = ZoneSBFormValues['legs'][0];
...
export default function ZoneSBPageContent() {
  ...
  const [comparisonHistory, setComparisonHistory] = useState<ZoneSBFormValues[]>([]);
  ...
  const saveToHistory = (data: ZoneSBFormValues) => {
    try {
      if (typeof window !== 'undefined') {
        const dataWithDate = { ...data, date: new Date() };
        const storedHistory = localStorage.getItem('zoneSbComparisonHistory');
        let history: ZoneSBFormValues[] = storedHistory ? JSON.parse(storedHistory) : [];
        ...
        localStorage.setItem('zoneSbComparisonHistory', JSON.stringify(newHistory));
  ...
  useEffect(() => {
    try {
      if (typeof window !== 'undefined') {
        const storedHistory = localStorage.getItem('zoneSbComparisonHistory');
        if (storedHistory) {
          const history: ZoneSBFormValues[] = JSON.parse(storedHistory);
          setComparisonHistory(history);
        }
      }
    } catch (e) {
      console.error('Failed to load zone sb comparison history:', e);
      if (typeof window !== 'undefined') {
        localStorage.removeItem('zoneSbComparisonHistory');
      }
    }
  }, []);
  ...
  const parseAndSetLegs = (csvText: string) => {
    ...
    const headerMap = {
        ...
        destZone: header.indexOf("destination zone"),
    ...
    lines.slice(1).forEach((line, index) => {
        ...
        const originZone = values[headerMap.originZone].toUpperCase();
        const destZone = values[headerMap.destZone].toUpperCase();
        const weight = parseFloat(values[headerMap.weight]);
        const price = parseFloat(values[headerMap.price]);
        
        if (!originZone || !destZone || isNaN(weight) || isNaN(price)) {
            errorCount++;
            return;
        }
  ...
  const handleDownloadTemplate = () => {
    ...
    link.setAttribute('download', 'zone_sb_template.csv');
  ...
  const handleExportCsv = () => {
    ...
    const link = document.createElement("a");
    const fileName = `ZoneSB_Analysis_${form.getValues('companyName') || 'Export'}_${format(new Date(), 'yyyy-MM-dd')}.csv`;
  ...
  const handleLoadHistory = (data: ZoneSBFormValues) => {
  ...
  const recalculateSingleLeg = async (leg: ZoneSBLeg): Promise<ServiceAnalysisResult[]> => {
      const servicesToCheck = form.getValues('selectedServices');
      
      const mockOrigin: PostcodeData = {
          suburb: leg.originZone.toUpperCase(),
          postcode: 0,
          state: leg.originZone.toUpperCase(),
          ipec: leg.originZone.toUpperCase(),
          prio: leg.originZone.toUpperCase(),
          pallet: leg.originZone.toUpperCase(),
          lat: 0,
          lng: 0,
          isZoneDirect: true
      } as any;
      
      const mockDest: PostcodeData = {
          suburb: leg.destinationZone.toUpperCase(),
          postcode: 0,
          state: leg.destinationZone.toUpperCase(),
          ipec: leg.destinationZone.toUpperCase(),
          prio: leg.destinationZone.toUpperCase(),
          pallet: leg.destinationZone.toUpperCase(),
          lat: 0,
          lng: 0,
          isZoneDirect: true
      } as any;
      ...
      const serviceAnalysisPromises = servicesToCheck.map(async (serviceName) => {
          ...
          for (const sb of spendBandsToCheck) {
              const freightFormValuesForCalc: FreightFormValues = {
                  spendBand: sb, originLocation: mockOrigin, destinationLocation: mockDest, originQuery: leg.originZone, destinationQuery: leg.destinationZone,
                  ...
                  accountTransferRequired: false, afterHoursCollection: false, afterHoursDelivery: false, publicHolidayService: false,
                  bookInDeliveryRequired: false, dangerousGoodsConsignment: false, handUnloadRequired: false, routeViaMelbourne: false,
              };
      ...
      <Card className="shadow-xl print-hide">
        <CardHeader>
          <CardTitle className="text-2xl font-headline flex items-center">
            <MapIcon className="mr-2 h-7 w-7 text-primary" /> Zone SB (Rate Comparison)
          </CardTitle>
          <CardDescription>
            Enter competitor freight legs by Zone to see which TGE spend band is most competitive.
          </CardDescription>
        </CardHeader>
      </Card>
                  ...
                  <Card key={field.id} className="p-4 bg-muted/30">
                    <div className="grid grid-cols-1 lg:grid-cols-[2fr_2fr_1fr_1fr_auto] gap-4 items-end">
                      <div className="space-y-1">
                        ...
                        />
                        {form.formState.errors.legs?.[index]?.originZone && (
                            <p className="text-red-500 text-xs mt-1">{form.formState.errors.legs[index]?.originZone?.message}</p>
                        )}
                      </div>
                      <div className="space-y-1">
                        <Label>Destination Zone</Label>
                        <Input
                          placeholder="e.g. VIC1"
                        ...
                        />
                        {form.formState.errors.legs?.[index]?.destinationZone && (
                            <p className="text-red-500 text-xs mt-1">{form.formState.errors.legs[index]?.destinationZone?.message}</p>
                        )}
                      </div>
                       <div className="space-y-1">
