...
import { getAllowedServices, DEFAULT_SERVICE_PERMISSIONS, isServiceEnabledForCompany } from '@/lib/types';
...
export default function DashboardCalculator() {
  ...
  const { globalSpendBands, serviceSettings, surchargeDefinitions, servicePermissions, updateGroupFuelSurcharge } = useSettings();
  const { getRateFile, getAllRateFiles, isLoading: isLoadingRates, areOurRatesLoaded, pezoneData } = useRateOverrides();
  const { user, profile, company, actualRole, loading: authLoading } = useAuth();
  ...
  // Added missing state from user's implied logic
  const [isUpdatingFuel, setIsUpdatingFuel] = useState(false);

  // Fetch recent history for the current user to fill space and add value
  ...
  const allowedServices = useMemo(() => {
    ...
    const roleAllowed = getAllowedServices(profile?.role || null, permissions);
    return roleAllowed.filter(s => isServiceEnabledForCompany(s, company, actualRole));
  }, [profile?.role, actualRole, servicePermissions, company]);
  ...
  // REHYDRATION LOGIC: Pick up state from session storage (Replicate or AI Chat)
  useEffect(() => {
    if (typeof window !== 'undefined' && !isLoadingRates) {
        ...
        if (rehydrateStr) {
            try {
                ...
                toast({ title: "Form Rehydrated", description: "Audit trail parameters loaded." });
                
                // Trigger calculation if rehydration is complete
  ...
  }, [reset, handleSubmit, toast, isLoadingRates]);
  ...
  const runCalculation = async (data: DashboardCalculatorFormValues) => {
    ...
    const calculatorState: any = {
      ...
      additionalPercentageType: data.additionalPercentageType, 
      additionalPercentageCustom: data.additionalPercentageCustom,
    };
    ...
    try {
        ...
        console.error("Quick Quote calculation failed:", e);
    ...
    <>
      <Card className="shadow-lg flex flex-col h-full">
        ...
        <CardContent className="space-y-6 flex-grow">
          <form onSubmit={handleSubmit(runCalculation)} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
              <div className="space-y-1.5">
                <Label htmlFor="dash-spendBand" className="text-[10px] uppercase font-black text-muted-foreground">Spend Band</Label>
                <Controller
                  name="spendBand"
                  control={control}
                  render={({ field }) => (
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger id="dash-spendBand" className="h-9">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="dash-originQuery" className="text-[10px] uppercase font-black text-muted-foreground">Origin</Label>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="dash-destinationQuery" className="text-[10px] uppercase font-black text-muted-foreground">Destination</Label>
                <LocationAutocomplete
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-4 items-end">
               <div className="space-y-1.5">
                <Label htmlFor="dash-weight" className="text-[10px] uppercase font-black text-muted-foreground">Weight (kg)</Label>
                <Input id="dash-weight" type="number" {...form.register('weight')} placeholder="0" onFocus={(e) => e.target.select()} className="h-9" />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="dash-quantity" className="text-[10px] uppercase font-black text-muted-foreground">Qty</Label>
                <Input id="dash-quantity" type="number" {...form.register('quantity')} placeholder="1" onFocus={(e) => e.target.select()} className="h-9" />
              </div>
          ...
          {/* Recent Calculations List to fill empty space and add value */}
