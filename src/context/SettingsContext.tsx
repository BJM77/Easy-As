...
const PREDEFINED_SURCHARGES_INITIAL: SurchargeDefinition[] = [
  ...
  { id: 'manual_handling_120_179', name: 'Manual Handling Fee (L/W 120-179.9cm)', type: 'fixed_per_shipment', defaultValue: 16.50, isConfigurablePerService: true, isPredefined: true, applicableServices: NON_PALLET_SERVICES },
];
...
export const SettingsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [showLcpRates, setShowLcpRates] = useState(false);
  ...
  useEffect(() => {
    const updateTimezones = () => {
      const newTimezones: Record<string, TimezoneInfo> = {};
      Object.entries(ALL_TIMEZONES).forEach(([key, tzInfo]) => {
        ...
        newTimezones[key] = { label: tzInfo.label, tz: tzInfo.tz, time };
      });
      setTimezones(newTimezones);
    };

    updateTimezones();
    const interval = setInterval(updateTimezones, 1000); // Update every second to ensure minute changes are caught promptly
  ...
  useEffect(() => {
    async function loadFirebaseSettings() {
      ...
      try {
        const snap = await getDoc(doc(firestore, 'settings', 'global_surcharges'));
        if (snap.exists()) {
  ...
  const saveSettingsToServer = useCallback(async (password: string, overrides?: any) => {
    ...
    if (firestore) {
      try {
        ...
        await setDoc(doc(firestore, 'settings', 'global_surcharges'), payload, { merge: true });
        toast({ title: 'Settings saved permanently to Firebase.' });
        return true;
      } catch (err: any) {
    ...
    toast({ title: 'Settings saved locally for session (Firebase disconnected).' });
    return true;
  }, [toast, firestore, serviceSettings, surchargeDefinitions, standardFuelSurcharge, priorityFuelSurcharge, palletFuelSurcharge, globalSecuritySurchargePercent]);
  ...
  const updateGroupOtherSurcharge = useCallback((groupKey: SurchargeConfigGroupKey, surchargeId: string, value: number, enabled: boolean) => {
    setServiceSettings(prev => {
      const newSettings = [...prev];
      const servicesInGroup = SURCHARGE_CONFIG_GROUPS[groupKey].services;
      
      servicesInGroup.forEach(serviceName => {
        const existingIdx = newSettings.findIndex(s => s.id === serviceName);
        if (existingIdx >= 0) {
          const service = { ...newSettings[existingIdx] };
          const surchargeIdx = service.surcharges.findIndex(s => s.surchargeId === surchargeId);
          if (surchargeIdx >= 0) {
            service.surcharges[surchargeIdx] = { ...service.surcharges[surchargeIdx], value, enabled };
          } else {
            service.surcharges.push({ surchargeId, value, enabled });
          }
          newSettings[existingIdx] = service;
        } else {
          newSettings.push({
            id: serviceName,
            name: String(serviceName),
            fuelSurchargePercent: 0,
            surcharges: [{ surchargeId, value, enabled }]
          });
        }
      });
      return newSettings;
